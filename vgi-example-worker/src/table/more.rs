...
impl TableProducer for ProjectedData {
    ...
    fn encode_resume(&self) -> Vec<u8> {
        resume::pack(&[self.remaining, self.current])
    }
    fn restore_resume(&mut self, bytes: &[u8]) {
...
impl TableProducer for OneShot {
    ...
    fn encode_resume(&self) -> Vec<u8> {
        // `batch` is regenerated by `producer(&params)`; only the done flag
        // (whether the single batch was already emitted) must round-trip.
        resume::pack(&[if self.batch.is_none() { 1 } else { 0 }])
    }
    fn restore_resume(&mut self, bytes: &[u8]) {
        if let Some(v) = resume::unpack(bytes, 1) {
            if v[0] != 0 {
