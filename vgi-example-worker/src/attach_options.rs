...
fn option_defs() -> Vec<(&'static str, &'static str, DataType, ArrayRef)> {
    let list_int64 = {
        ...
        Arc::new(ListArray::new(field, OffsetBuffer::new(vec![0i32, 3].into()), values, None)) as ArrayRef
    };
    let struct_default = {
    ...
    vec![
        ("opt_bool", "Boolean option", DataType::Boolean, Arc::new(BooleanArray::from(vec![true]))),
        ("opt_int8", "int8", DataType::Int8, Arc::new(Int8Array::from(vec![-8]))),
        ("opt_int16", "int16", DataType::Int16, Arc::new(Int16Array::from(vec![-16]))),
        ("opt_int32", "int32", DataType::Int32, Arc::new(Int32Array::from(vec![-32]))),
        ("opt_int64", "int64", DataType::Int64, Arc::new(Int64Array::from(vec![-64]))),
        ("opt_uint8", "uint8", DataType::UInt8, Arc::new(UInt8Array::from(vec![8]))),
        ("opt_uint16", "uint16", DataType::UInt16, Arc::new(UInt16Array::from(vec![16]))),
        ("opt_uint32", "uint32", DataType::UInt32, Arc::new(UInt32Array::from(vec![32]))),
        ("opt_uint64", "uint64", DataType::UInt64, Arc::new(UInt64Array::from(vec![64]))),
        ("opt_float32", "float32", DataType::Float32, Arc::new(Float32Array::from(vec![1.5]))),
        ("opt_float64", "float64", DataType::Float64, Arc::new(Float64Array::from(vec![2.5]))),
        ("opt_string", "UTF-8 string", DataType::Utf8, Arc::new(StringArray::from(vec!["hello"]))),
        ("opt_blob", "Binary blob", DataType::Binary, Arc::new(BinaryArray::from(vec![&[0u8, 1, 2][..]]))),
        ("opt_date", "Date", DataType::Date32, Arc::new(Date32Array::from(vec![20567]))),
        ("opt_time", "Time of day", DataType::Time64(TimeUnit::Microsecond),
            Arc::new(Time64MicrosecondArray::from(vec![45_296_000_000]))),
        ("opt_timestamp", "Naive timestamp", DataType::Timestamp(TimeUnit::Microsecond, None),
            Arc::new(TimestampMicrosecondArray::from(vec![ts]))),
        ("opt_timestamp_tz", "Timestamp with UTC tz",
            DataType::Timestamp(TimeUnit::Microsecond, Some("UTC".into())),
            Arc::new(TimestampMicrosecondArray::from(vec![ts]).with_timezone("UTC"))),
        ("opt_decimal", "Decimal(18,4)", DataType::Decimal128(18, 4), decimal),
        ("opt_list", "List of int64", DataType::List(Arc::new(Field::new("item", DataType::Int64, true))), list_int64),
        ("opt_struct", "Struct",
            DataType::Struct(vec![Field::new("a", DataType::Int64, true), Field::new("b", DataType::Utf8, true)].into()),
            struct_default),
    ]
}
...
fn echo_schema() -> SchemaRef {
    Arc::new(Schema::new(
        option_defs().iter().map(|(n, _, t, _)| Field::new(*n, t.clone(), true)).collect::<Vec<_>>(),
    ))
}
...
pub fn catalog() -> vgi::catalog::CatalogModel {
    let specs: Vec<Vec<u8>> = option_defs()
        ...
        .collect();
    let default = default_batch().ok().and_then(|b| vgi::ipc::write_batch(&b).ok());
...
impl TableFunction for EchoAttachOptionsFunction {
    ...
    fn on_bind(&self, _p: &BindParams) -> Result<BindResponse> {
        Ok(BindResponse { output_schema: echo_schema(), opaque_data: Vec::new() })
    }
    fn cardinality(&self, _p: &BindParams) -> Option<vgi::table_function::TableCardinality> {
        Some(vgi::table_function::TableCardinality { estimate: Some(1), max: Some(1) })
    }
